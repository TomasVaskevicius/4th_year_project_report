...
but remain flexible if some parts need to modified for custom needed.
...
reassure us, that
choosing Poisson process simulation, Markov kernel and deterministic flow as policies in
our PDMP framework's design makes perfect sense.
...
\end{align*}
We can then simulate our PDMP started at $x$ follows:
\begin{enumerate}
...
The opportunity for more efficient simulation arise
from the Poisson process simulation by the composition method introduced in
...
which is why we have $\forall S \in \mathcal{P}(\mathbb{N})\,S \subset \text{Dep}_{\phi}(S)$.)
